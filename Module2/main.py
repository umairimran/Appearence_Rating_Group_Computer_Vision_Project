...
import shutil  # Add this import for the rmtree function
...
def get_best_person_crop(cv2_image, detections):
    """
    From YOLO detections, pick the best (most visible, highest confidence) person and return the cropped image.
    """
    best_crop = None
    best_conf = 0
    ...
    for det in detections:
        ...
        if aspect_ratio < 1.3 or aspect_ratio > 4.0:
            continue
        if height / h_img < 0.25:
            continue

        # Check if this is the best so far
        if conf > best_conf:
            best_crop = cv2_image[y1:y2, x1:x2]
            best_conf = conf
...
def isolate_people_from_faces(image, face_boxes, save_dir="cropped_people"):
    # Clear the directory if any data is present in it
...
def process_and_crop_faces_from_folder(
    ...
    # === 1. Clean or create output folder ===
    if os.path.exists(output_folder):
        shutil.rmtree(output_folder)
    os.makedirs(output_folder)

    # === 2. Validate face detection function ===
    if detect_faces_fn is None:
        raise ValueError("You must provide a face detection function via detect_faces_fn")

    # === 3. Process each image in the input folder ===
    for filename in os.listdir(input_folder):
        ...
        # === 4. Detect faces
        face_boxes = detect_faces_fn(image, min_conf=min_conf)

        # Filter based on confidence only
        valid_faces = [face for face in face_boxes if face.get('conf', 0) >= min_conf]

        if not valid_faces:
            print(f"⚠️ No valid face found in: {filename}")
            continue

        # === 5. Choose the most confident face
        best_face = max(valid_faces, key=lambda f: f['conf'])
        x1, y1, x2, y2 = best_face['bbox']
        face_crop = image[y1:y2, x1:x2]

        # === 6. Save the cropped face
        out_name = f"{os.path.splitext(filename)[0]}_bestface.jpg"
        out_path = os.path.join(output_folder, out_name)

        cv2.imwrite(out_path, face_crop)
        print(f"✅ Saved: {out_path}")

    print("✅ All best faces cropped and saved.")



def run_pipeline_from_image(image_path):
    # Load the image first
    input_image = Image.open(image_path)
    detect_people_and_faces(input_image)

    # Detect faces and show the face(s) on the resultant image
    input_image = Image.open(image_path)
    process_and_crop_faces_from_folder("cropped_people", "cropped_faces", detect_faces, 0.4)
